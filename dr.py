...
async def on_ready():
    ...
    print("------------DARYA-------------")
    await client.change_presence(game=discord.Game(name='dr?ajuda', type=1, url='https://www.twitch.tv/daryabot'),status='streaming')
...
async def on_message(message):
    if message.content.lower().startswith("dr?ban"):
        try:
            ...
            embed1.set_image(url="https://cdn.discordapp.com/attachments/461479588481073162/462649531041972224/f9c305fb.gif")
            await client.ban(user)
            embed1.add_field(name="<:revolving_hearts:462189330702532618> ***Darya***",value='{}***Foi Banido Com Sucesso Senpai*** **{}**'.format(user.mention, author))
            embed1.set_footer(text=client.user.name, icon_url=client.user.avatar_url)
    ...
    if message.content.lower().startswith("dr?mutar"):
        if not message.author.server_permissions.administrator:
            return await client.send_message(message.channel, '<:error:455028754008375296> **|** Você não tem permissões suficiente.')
        user = message.mentions[0]
        ...
        try:
         ...
         embed1 = discord.Embed(color=0xff00eb)
         embed1.add_field(name="<:mute:455719792037330965> Mute", value='{}\n``` Foi mutado pelo administrador:``` **{}**'.format(user.mention,author))
         embed1.set_footer(text=client.user.name, icon_url=client.user.avatar_url)
         await client.send_message(message.channel, embed=embed1)
        except AttributeError:
         embed1 = discord.Embed(color=0xff00eb)
         embed1.add_field(name="<:error:455028754008375296> Mute", value='***Infelizmente o servidor não tinha o cargo*** ***Dayra-Mute***, o cargo foi criado, digite o comando novamente.')
         embed1.set_footer(text=client.user.name, icon_url=client.user.avatar_url)
         await client.send_message(message.channel, embed=embed1)
    ...
    if message.content.lower().startswith("dr?desmultar"):
        ...
        embed1 = discord.Embed(color=0xff00eb)
        embed1.add_field(name="<:mute:455719792037330965> Mute",value='{} Foi desmultado pelo administrador: **{}**'.format(user.mention, author))
        embed1.set_footer(text=client.user.name, icon_url=client.user.avatar_url)
        await client.send_message(message.channel, embed=embed1)
    ...
    if message.content.startswith('dr?server'):
        ...
        serverinfo_embed.add_field(name="<:triangular_flag_on_post:442116432386326529> ID:", value=message.server.id, inline=True)
        serverinfo_embed.add_field(name="<:books:442111670584606730> Cargos:", value=len(message.server.roles), inline=True)
        serverinfo_embed.add_field(name="<:bust_in_silhouette:440060372213432321> Membros:", value=len(message.server.members), inline=True)
        serverinfo_embed.add_field(name="<:date:442114695558856704> Criado em:", value=message.server.created_at.strftime("%d %b %Y %H:%M"))
        serverinfo_embed.set_footer(text=client.user.name, icon_url=client.user.avatar_url)
    ...
    if message.content.lower().startswith('dr?clima'):
        ...
        try:
           ...
           await client.send_message(message.channel, '<:__:444646408365146153> `Infelizmente está cidade não é existente`')
        ...
        try:
            ...
            avatarembed2 = discord.Embed(title="",color=0xff00eb,description="[Clique aqui para baixar a imagem](" + message.author.avatar_url + ")")
    ...
    if message.content.lower().startswith('dr?py'):
        ...
        await client.send_message(message.channel, '<:python:445039606157541377> ' + f"{message.author.mention}" + ' enviou o segunte código:' + '\n```python\n{}\n```'.format(msg))
    ...
    if message.content.lower().startswith('dr?memoria'):
     ...
     pingm1 = discord.Embed(title='Memória utilizada pelo bot:', description='<:memoria:444955571351977994> `2{}mb`'.format(string), color=0x7289DA)
    ...
    if message.content.lower().startswith('dr?ping'):
     ...
     pingm1 = discord.Embed(title='Ping:', description=f"<:ping:444955571515555850> `{int(ping)} ms`", color=0xff00eb)
     await client.edit_message(pingm0, embed=pingm1)
    ...
    if message.content.lower().startswith('dr?servidor'):
        try:
            ...
            informacao_add = "\n:sunglasses:  " + str(emojis) + " Emojis" + "\n:robot:  " + str(
                cargos) + " Cargos" + "\n:bookmark_tabs: Verificação : " + str(verificao) + " " + "\n:earth_americas: Localização : " + str(
                localizacao) + " "
    ...
    if message.content.lower().startswith('dr?msg'):
        ...
        s = 0
        for member in x:
            embed1 = discord.Embed(title="Darya", url="", color=0xff00eb,description=' <@{}> {}'.format(member.id, msg))
    ...
    if message.content.startswith('dr?criador'):
        await client.send_message(message.channel,"***Meu Criador*** <@403395784751448075> <:revolving_hearts:462212168964898817>")
